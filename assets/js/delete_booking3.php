...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $dendadd30 = new DateTime($end);
        ...
        $secondSlot = trim($start) . '-' . $dstartadd30->format("h:i A");

        $thirdSlot = trim($start) . '-' . $dstartadd60->format("h:i A");

        $fourthSlot = $dstartadd30->format("h:i A") . '-' . $dstartadd60->format("h:i A");
    
        $fifthSlot = $dstartadd60->format("h:i A") . '-' . trim($end);

        $sixthSlot = $dstartadd60->format("h:i A"). '-' . $dstartadd90->format("h:i A");

        $seventhSlot = $dstartadd90->format("h:i A") . '-' . trim($end);

    
$servername = "localhost";
$username = "id20034203_root";
$password = "C@pston3"; 
$dbname = "id20034203_josonpereybooking";
    ...
    $stmt = $conn->prepare("DELETE FROM bookings WHERE date = ? AND timeslot = ? AND name = ?");
