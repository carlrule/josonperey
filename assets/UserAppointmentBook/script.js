...
var database = firebase.database(); 
  ...
  const step1 = "assets/UserAppointmentBook/step1.html"
  const step2 = "assets/UserAppointmentBook/step2.html"
  const step3 = "assets/UserAppointmentBook/calendar.php";
  const step3b = "assets/UserAppointmentBook/calendarb.php";
  const step4 = "assets/UserAppointmentBook/step4.html"; 
  const step5 = "assets/UserAppointmentBook/payment.html"; 
  const step6 = "assets/UserAppointmentBook/step5.html"; 
      ...
      const receivedValue = event.data; //Data na marereceived 
        ...
        const lastCheckedRadio = sessionStorage.getItem('lastCheckedRadio');
            if (lastCheckedRadio === 'radio1') {
              iframe.classList.add('medium6'); 
              iframe.classList.add('medium4'); 
              iframe.classList.remove('medium3');
              
            } else if (lastCheckedRadio === 'radio2') {
              iframe.classList.add('medium4'); 
            iframe.classList.remove('medium3');
            iframe.classList.remove('medium6');
            }
    ...
    if (currentStep === 4 && e.target.id === "next") {
      ...
      if (currentStep === 2) {
        ...
        document.getElementById("time_step3").style.display = "none";
          ...
          iframe.classList.add('medium6'); // Add large class
          iframe.classList.remove('medium3');
        ...
        document.getElementById("services_step2").style.display = "none";
          ...
          iframe.src = step5;
          iframe.classList.add('medium5'); // Add large class
          iframe.classList.remove('medium4');
          iframe.classList.remove('medium6');
      ...
      .then(() => {
          console.log("Data saved successfully!"); // Log message if data is saved successfully
      })
      .catch(error => {
          console.error("Error saving data:", error); // Log error if there's an issue saving data
      });
      
           sendMessage(`Hi ${firstname + ' ' + lastname},
    
           We are pleased to inform you that your booking has been accepted. Your appointment is confirmed for the following date and time:
...
function performSpecificActions(time, date) {
    ...
    if (durationMinutes === 30) {
        const data = new URLSearchParams();
            ...
            fetch('assets/UserAppointmentBook/delete_booking.php', {
                method: 'POST',
                body: data,
            })
            .then(response => response.text())
        ...
        const data = new URLSearchParams();
            ...
            data.append('timeslot', time);
            data.append('name', name);
            fetch('assets/UserAppointmentBook/delete_booking2.php', {
                method: 'POST',
                body: data,
            })
            .then(response => response.text())
            ...
            data.append('timeslot', time);
            data.append('name', name);
            fetch('assets/UserAppointmentBook/delete_booking3.php', {
                method: 'POST',
                body: data,
            })
            .then(response => response.text())
    ...
    function performSpecificActionsb(time, date) {
        ...
        if (durationMinutes === 30) {
            const data = new URLSearchParams();
                ...
                data.append('timeslot', time);
                data.append('name', name);
                fetch('assets/UserAppointmentBook/delete_bookingb.php', {
                    method: 'POST',
                    body: data,
                })
                .then(response => response.text())
            ...
            const data = new URLSearchParams();
                ...
                data.append('timeslot', time);
                data.append('name', name);
                fetch('assets/UserAppointmentBook/delete_booking2b.php', {
                    method: 'POST',
                    body: data,
                })
                .then(response => response.text())
                ...
                data.append('timeslot', time);
                data.append('name', name);
                fetch('assets/UserAppointmentBook/delete_booking3b.php', {
                    method: 'POST',
                    body: data,
                })
                .then(response => response.text())
